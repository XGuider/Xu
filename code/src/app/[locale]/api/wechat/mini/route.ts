...
export async function POST() {
  try {
    ...
    console.log('[api/wechat/mini POST] 开始抓取小程序数据...');

    const { stdout, stderr } = await execAsync('python', [pythonScript], {
      cwd: projectRoot,
      env: {
        ...process.env,
        WECHAT_CRAWL_KIND: 'mini',
    ...
    if (stderr) {
      console.warn('[api/wechat/mini POST] 警告:', stderr);
    }

    console.log('[api/wechat/mini POST] 抓取完成:', stdout);
    ...
    console.error('[api/wechat/mini POST] 抓取失败:', error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      { success: false, message: `抓取小程序数据失败: ${errorMessage}` },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await deleteById('mini', id);
    if (deleted) {
    ...
    console.error('[api/wechat/mini DELETE]', error);
    return NextResponse.json(
      { success: false, message: '删除小程序数据失败' },
      { status: 500 },
    );
  }
}
