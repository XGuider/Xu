...
- **开发体验**: 完整的 TypeScript 支持、ESLint + Prettier、Knip 依赖检查
...
```bash
npm run build   # 或 npm run build:next
npm start
```

完整环境变量与各平台部署步骤见下文 **「🚀 部署」** 章节。
...
- **Node.js**：20.x（与 `package.json` 的 `engines` 及仓库 `.github/workflows/deploy.yml` 一致）。
- **应用根目录**：本仓库的应用代码在 **`code/`** 目录；以下命令均在 `code` 下执行（`cd code`）。

### 环境变量

复制 [`code/.env.example`](.env.example) 为 `.env` 或 `.env.local` 并按需填写。修改任意 **`NEXT_PUBLIC_*`** 变量后：开发环境需**重启** `npm run dev`；生产环境需**重新执行** `npm run build` 后再启动。

| 变量 | 说明 |
|------|------|
| `NEXT_PUBLIC_APP_URL` | 站点对外访问 URL |
| `NEXT_PUBLIC_APP_VERSION` | 可选，页脚版本号覆盖 |
| `NEXT_PUBLIC_POSTHOG_KEY` / `NEXT_PUBLIC_POSTHOG_HOST` | 可选，PostHog |
| `NEXT_PUBLIC_GA_MEASUREMENT_ID` | 可选，Google Analytics 4 |
| `NEXT_PUBLIC_BAIDU_TONGJI_ID` | 可选，百度统计 |
| `NEXT_PUBLIC_SENTRY_*`、`SENTRY_*` | 可选，Sentry |
| `ARCJET_KEY` | 可选，Arcjet |
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`、`CLERK_SECRET_KEY` | 可选，Clerk |

当前业务数据来自 `data/categories.json` 与 `data/tools.json`，**无需**配置数据库即可运行；`.env.example` 中的数据库相关项可忽略。
...
默认监听 **3000** 端口。构建产物在 `.next`，请与 `public/`、`data/` 一并保留以便运行。

### GitHub Actions（CI）

工作流：[`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml)。在 **`code`** 工作目录下依次执行：`npm ci` → `npm run lint` → `npm run check:types` → `npm run build:next`（`NODE_ENV=production`）。文件中注释掉的 Vercel 部署步骤可按需启用并配置 Secrets。

### Vercel（推荐）

1. 在 Vercel 导入 Git 仓库，将 **Root Directory** 设为 **`code`**。
2. **Install Command**：`npm ci`；**Build Command**：`npm run build` 或 `npm run build:next`；**Start Command**：`npm start`。
3. 在 Project → Environment Variables 中配置上表变量。
4. 使用 Vercel 对 Next.js 的默认集成即可；**不要**把应用误配成「仅上传 `.next` 目录的静态站点」。

`data/` 下 JSON 会随仓库一起部署；更新数据后重新触发部署即可。
...
若启用 `next.config.ts` 中的 `output: 'standalone'`，可按 [Next.js 官方说明](https://nextjs.org/docs/app/building-your-application/deploying#docker-image) 精简运行镜像。

### Netlify 与其他平台

在 Netlify 上托管 Next.js 15 请使用官方 **Next.js** 集成（或 OpenNext 等方案），**不要**将「发布目录」错误地设为单独的 `.next` 文件夹。详见 [Netlify Next.js 文档](https://docs.netlify.com/frameworks/next-js/overview/)。
...
- ✅ **数据存储**: 本地JSON文件，简单高效
- ✅ **质量工具**: ESLint、TypeScript、`npm run check:deps`（Knip）
